...
const News = (props) => {
  ...
  const capitalizeFirstLetter = (string) => {
    ...
    useEffect(() => {
      document.title = `ATG - ${capitalizeFirstLetter(props.category)}`;
      updateNews();
    }, [])
    ...
    return (
      <>
          <h3 className='text-center' style={{margin: "35px 0px"}}> Top {capitalizeFirstLetter(props.category)} Headlines</h3>
           {loading && <Spinner />} 
